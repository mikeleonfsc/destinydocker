...
<c:set var="form" value="<%=request.getAttribute(DCPITitleDetailsForm.FORM_NAME)%>"/>

<table id="<%= DCPITitleDetailsForm.TABLE_BIB_REVIEWS %>" cellpadding="5" cellspacing="0" border="0" width="100%">
<c:choose>
<c:when test="<%= form.getReviews().getTotalReviews() == 0%>">
    <tr>
        <td width="100%" align="center" class="ColRowBold">
            <%=MessageHelper.formatMessage("bibreview_ThereAreNoReviews")  %>
        </td>
    </tr>
</c:when>
<c:otherwise>
    ...
    <tr>
        <td>
            <table id="<%= DCPITitleDetailsForm.TABLE_REVIEW_SUMMARY %>" border="0">
                <tr>
                    <td class="ColRowBold">
    ...
    <c:if test="${! empty reviews.reviewsWithoutComments}">
        ...
        <tr>
            <td width="100%">
                <table id="<%= DCPITitleDetailsForm.TABLE_REVIEWS_NO_COMMENTS %>" border="0" width="100%">
                    <tr>
                        ...
                        <td class="SmallColRow tdAlignRight">
                            <c:choose>
                                <c:when test="<%=form.isShowAllReviews() %>">
                                    <a href="<%=form.getShowLessMoreReviewLink() %>" class="DetailLink" id="<%=DCPITitleDetailsForm.ID_SHOW_LESS %>">
                                ...
                                <c:otherwise>
                                    <a href="<%=form.getShowLessMoreReviewLink() %>" class="DetailLink" id="<%=DCPITitleDetailsForm.ID_SHOW_MORE %>">
                            ...
                            <tr>
                                ...
                                <td class="tdAlignRight">
                                    <c:if test="<%= form.getStore().getSiteID().equals(reviewNoComments.getSiteID()) %>">
                                        <base:link permission="<%= Permission.CAT_APPROVE_DELETE_LIBRARY_REVIEWS %>" href='<%= form.getDeleteReviewLink() + "&reviewID=" + reviewNoComments.getReviewID() %>' id="<%=DCPITitleDetailsForm.ID_DELETE_REVIEW + reviewNoComments.getReviewID() %>">
    ...
    <tr>
        <td width="100%">
            <table id="<%= DCPITitleDetailsForm.TABLE_REVIEWS_WITH_COMMENTS %>" border="0" width="100%">
                    ...
                    <tr>
                        ...
                        <td class="tdAlignRight">
                            <c:if test="<%= form.getStore().getSiteID().equals(review.getSiteID()) %>">
                                <base:link permission="<%= Permission.CAT_APPROVE_DELETE_LIBRARY_REVIEWS %>" href='<%= form.getDeleteReviewLink() + "&reviewID=" + review.getReviewID() %>' id="<%=DCPITitleDetailsForm.ID_DELETE_REVIEW + review.getReviewID() %>">
                                    <base:image src="/buttons/small/delete_70.gif" alt='<%= MessageHelper.formatMessage("delete") %>'/>
                                </base:link>
                            </c:if>
                        </td>
