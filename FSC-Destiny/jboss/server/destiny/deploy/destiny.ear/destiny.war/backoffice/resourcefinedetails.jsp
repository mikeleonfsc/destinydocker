...
<%@page import="com.follett.fsc.common.CurrencyValue"%><table width="100%" id="<%=ResourceFineDetailsForm.TABLE_MAIN %>">
    <tr>
        <td colspan="<%=tableWidth-1 %>" class="TableHeading"><%=form.getReportHeader()  %></td>
        <td class="tdAlignRight">
            <% if (!form.isPrinterFriendly()) { %>
                <base:link page="<%=form.getPrinterFriendlyLink() %>" target="printerFriendly" id="<%=ResourceFineDetailsForm.ID_PRINTER_FRIENDLY %>"><base:image src="/buttons/small/printerfriendly.gif" alt="Printer Friendly"/></base:link>
            <% } else { %>
                &nbsp;
            <% } %>
        </td>
...
%>
    <tr>
        ...
        <% } %>
        <td class="ColRow tdAlignRight"><%=row.getItemBarcodeOrDeleted()%></td>
