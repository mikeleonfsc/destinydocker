...
REM ********************************************************************************
REM * process_%CONTEXT%_assets.bat
REM *
REM * Usage:
REM *
...
REM * Set your consortium context here
IF "%CONTEXT%"=="" SET CONTEXT=yourConsortiumDatabaseName
...
IF "%FINAL%"=="" SET FINAL=..\assetdata\destiny_%CONTEXT%_asset_import_final.xml
REM This creates a copy of your asset CSV file for importing
IF "%TEMPCSVFILE%"=="" SET TEMPCSVFILE=..\assetdata\destiny_%CONTEXT%_assets.csv
...
IF "%LOGFILE%"=="" SET LOGFILE=..\automate\process_%CONTEXT%_asset_import.log
...
IF "%UPDATE%"=="" SET UPDATE=.\update_%CONTEXT%_asset_import.properties
...
IF "%RAWFILE%"=="" SET RAWFILE=..\assetdata\destiny_%CONTEXT%_asset_import.csv
...
echo Copying Asset Import data file from %SOURCEFULLFILE% to ..\assetdata\%CONTEXT%\%TEMPCSVFILE%   >>%LOGFILE%
...
REM * ImportAssets configurationFile assetFileName contextName assetMatching itemMatching
REM * NOTE: Comment out the following 3 lines if you use the Schedule Import Assets
REM *       from the Destiny application.
REM ********************************************************************************
cd ..\bin
call ImportAssets.exe %UPDATE% %FINAL% %CONTEXT% %assetMatch% %itemMatch%  >>%LOGFILE%
