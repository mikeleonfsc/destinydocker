...
REM ********************************************************************************
REM * process_%CONTEXT%_patrons.bat
REM * 
...
REM *              variables as needed for the member (i.e. file names...process_<context>_patrons.bat;
REM *              destiny_<context>_patrons.xml, etc...)    
...
REM * 10/26/2011 - DL (Development) - Modifed based on defect found from technical support for JTRAC: DESTINY-2092
REM *
...
REM * 
REM *  process_context_patrons [intputfile] [NOCOMPARE] [NOLOAD]
REM * 
...
IF "%LOGFILE%"=="" SET LOGFILE=..\automate\%CONTEXT%\process_%CONTEXT%_patrons.log
IF "%FINAL%"=="" SET FINAL=..\patdata\%CONTEXT%\destiny_%CONTEXT%_patrons.xml
IF "%PROP%"=="" SET PROP=.\%CONTEXT%\map_%CONTEXT%_patrons.properties
IF "%UPDATE%"=="" SET UPDATE=.\%CONTEXT%\update_%CONTEXT%_patrons.properties
IF "%SORTED%"=="" SET SORTED=..\patdata\%CONTEXT%\sorted_%CONTEXT%_patrons.csv
...
IF "%RAWFILE%"=="" SET RAWFILE=..\patdata\destiny_%CONTEXT%_patrons.csv
...
REM ********************************************************************************
REM cd %SOURCEDIR%
REM copy /Y *.csv patrons.csv
REM SET SOUREFILE=patrons.csv
...
echo Copying patron data file from %SOURCEFULLFILE% to ..\patdata\destiny_%CONTEXT%_patrons.csv   >>%LOGFILE%
...
REM ********************************************************************************
echo Saving %SORTED% to sorted_patrons.csv.001   >>%LOGFILE%
...
REM ********************************************************************************
echo Saving %FINAL% to destiny_patrons.xml.001   >>%LOGFILE%
...
REM ********************************************************************************
echo Saving %LOGFILE% to process_patrons.log.001   >>%LOGFILE%
