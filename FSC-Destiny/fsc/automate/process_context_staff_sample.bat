...
REM ********************************************************************************
REM * process_%CONTEXT%_staff.bat
REM *
...
REM *              variables as needed for the member (i.e. file names...process_<context>_staff.bat;
REM *              destiny_<context>_staff.xml, etc...)
...
REM * 
REM *  process_context_staff [intputfile] [NOCOMPARE] [NOLOAD]
REM * 
...
IF "%SOURCEFILE%"=="" SET SOURCEFILE=staff.csv
...
IF "%LOGFILE%"=="" SET LOGFILE=..\automate\%CONTEXT%\process_%CONTEXT%_staff.log
IF "%FINAL%"=="" SET FINAL=..\patdata\%CONTEXT%\destiny_%CONTEXT%_staff.xml
IF "%PROP%"=="" SET PROP=.\%CONTEXT%\map_%CONTEXT%_staff.properties
IF "%UPDATE%"=="" SET UPDATE=.\%CONTEXT%\update_%CONTEXT%_staff.properties
IF "%SORTED%"=="" SET SORTED=..\patdata\%CONTEXT%\sorted_%CONTEXT%_staff.csv
IF "%NOCHGXML%"=="" SET NOCHGXML=..\patdata\destiny_staff_nochange.xml
IF "%NOCHGUPD%"=="" SET NOCHGUPD=.\%CONTEXT%\update_%CONTEXT%_no_change.properties
IF "%RAWFILE%"=="" SET RAWFILE=..\patdata\destiny_%CONTEXT%_staff.csv
...
REM ********************************************************************************
REM cd %SOURCEDIR%
REM copy /Y *.csv staff.csv
REM SET SOUREFILE=staff.csv
...
echo Copying patron data file from %SOURCEFULLFILE% to ..\patdata\destiny_%CONTEXT%_staff.csv   >>%LOGFILE%
...
REM ********************************************************************************
echo Saving %SORTED% to sorted_staff.csv.001   >>%LOGFILE%
...
REM ********************************************************************************
echo Saving %FINAL% to destiny_staff.xml.001   >>%LOGFILE%
...
REM ********************************************************************************
echo Saving %LOGFILE% to process_staff.log.001   >>%LOGFILE%
